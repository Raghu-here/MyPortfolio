    ...
    <section id="skills" className="py-20 sm:py-28 lg:py-32">
      <div className="section-container">
        <div ref={heading.ref} style={heading.style}>
          <span className="text-xs sm:text-sm font-medium text-primary uppercase tracking-widest">Skills</span>
          <h2 className="font-heading text-2xl sm:text-3xl lg:text-4xl font-bold text-foreground mt-2">
            Technologies I work with
          </h2>
        </div>

        <div className="mt-10 sm:mt-12 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-6">
          {skills.map((skill, i) => (
...
const SkillCard = ({ skill, index }: { skill: typeof skills[0]; index: number }) => {
  const card = useScrollReveal({ variant: "fade-up", delay: index * 100 });

  return (
    <div
      ref={card.ref}
      style={{ ...card.style, boxShadow: 'var(--shadow-card)' }}
      className="hover-lift group flex flex-col items-center gap-3 p-6 sm:p-8 rounded-2xl bg-card border border-border cursor-default"
    >
